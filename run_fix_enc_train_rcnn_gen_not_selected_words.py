...
for l_1 in lamda_1:
	...
	for l_2 in lamda_2:
		for lr in [ 0.0005]:
			...
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu0,floatX=float32" python fix_enc_train_rcnn_gen_not_selected_words.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) +' --embedding ../word_vec.gz --load_rationale ../annotations.json --aspect ' + str(aspect) + \
				' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --debug '+ str(debug) +' --select_all ' +str(select_all) + ' --learning_rate '+str(lr)+' --load_model ' +_type+ "MODELS/"+model_file #+' --load_model '+ load_model_file  + ' --load_emb_only '+ str(load_emb_only)
			...
			# run_command+= ' >> '+ 'not_selected_words_fix_enc_train_rcnn_gen' + str(l_1) + str(l_2)+'.txt' 	
			print run_command
