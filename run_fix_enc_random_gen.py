...
covered_percentage = []
graph_data_file = '../graph_data/table_fix_enc_rand_gen.txt'
...
for select_all in select_all_array:
	for l_2 in lamda_2:
		for lr in [0.01]:#, 0.0005]:
			...
			# load_model_file = _type+"MODELS/"+model_file
			...
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu1,floatX=float32" python fix_enc_random_gen.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) +' --embedding ../word_vec.gz --load_rationale ../annotations.json --aspect ' + str(aspect) + \
