...
class Generator(object):
    ...
    def get_cat_emb(self,  i, embs_mid,):
        temp = embs_mid[i,:,:]
        for j in range(2*self.nneighbor):
            temp = T.concatenate([temp, embs_mid[i+j+1,:,:]], axis = 1) #along the len dimention
        return temp #T.mean(embs_mid[i: i+2*self.nneighbor+1,:,:], axis = 0)#theano.scan_module.until(embs_mid)
    ...
    def ready(self):
        ...
        # cat_embs = T.zeros((x.shape[0], x.shape[1], (2*nneighbor+1)*n_e))
        cat_embs = T.zeros((x.shape[0], x.shape[1], (2*self.nneighbor+1)*n_e))

#         for i in range(nneighbor):
# ...       temp = T.zeros((x.shape[1], (nneighbor-i)*n_e))
# ...       for j in range(nneighbor+1+i):
# ...             temp = T.concatenate([temp, embs[j,:,:]], axis = 1)
# ...     cat_embs = T.set_subtensor(cat_embs[i, :, :], temp)

        
        
        # cat_embs = T.set_subtensor(cat_embs[:, :, :], embs[:, :, :])
        for i in range(self.nneighbor):
            temp = T.zeros((x.shape[1], (self.nneighbor-i)*n_e))
            for j in range(self.nneighbor+i+1):
                temp = T.concatenate([temp, embs[j,:,:]], axis = 1) #along the len dimention
            cat_embs = T.set_subtensor(cat_embs[i, :, :], temp)

        
        cat_emb_middle, _ = theano.scan(
                    fn = self.get_cat_emb,
        ...
        cat_embs = T.set_subtensor( cat_embs[self.nneighbor:-self.nneighbor, :, :], cat_emb_middle[:,:,:])


        for i in range(self.nneighbor):
            temp = T.zeros((x.shape[1], (self.nneighbor-i)*n_e))
            for j in range(self.nneighbor+i+1):
                temp = T.concatenate([embs[-j-1,:,:], temp,], axis = 1) #along the len dimention
            cat_embs = T.set_subtensor(cat_embs[-i-1, :, :], temp)

        #     cat_embs = T.set_subtensor(cat_embs[-i-1, :, :],T.mean(embs[-i-1-self.nneighbor:,:,:], axis = 0))
...
class Model(object):
    ...
    def train(self, train, dev, test, rationale_data, trained_max_epochs = None):
        ...
        best_dev = 1e+2
        best_dev_e = 1e+2
        last_train_cat_cost = None
        last_dev_cat_cost = None
        ...
        for epoch_ in xrange(args.max_epochs ): # -50 when max_epochs  = 100 given
            ...
            while more:
                ...
                for i in xrange(N):
                    ...
                    cost, loss, sparsity_cost, bz, emb, gl2_e, gl2_g, probs2, cat_emb_gen = train_generator(bx, by)
                    print cat_emb_gen
                ...
                cur_train_cat_cost = train_cost / N
                #print(" end : ", cur_train_cat_cost )
                ...
                more = False
                if args.decay_lr and last_train_cat_cost is not None:
                    if cur_train_cat_cost > last_train_cat_cost*(1+tolerance):
                        more = True
                        say("\nTrain cost {} --> {}\n".format(
                                last_train_cat_cost, cur_train_cat_cost
                            ))
                    if dev and cur_dev_cat_cost > last_dev_cat_cost*(1+tolerance):
                        more = True
                        say("\nDev cost {} --> {}\n".format(
                                last_dev_cat_cost, cur_dev_cat_cost
                            ))
                
                if more:
                    more_counter += 1
                    print('MORE COUNTER: ', more_counter)
                    if more_counter>15 or lr_g<e-12: 
                        print ' more counter limit has passed or lr is too small'
                        exit() #more = False
                ...
                last_train_cat_cost = cur_train_cat_cost
                if dev: last_dev_cat_cost = cur_dev_cat_cost
                ...
                if dev:
                    ...
                    say(("\tsampling devg={:.4f}  mseg={:.4f}  cat_diffg={:.4f}" + 
