...
lamda_2 = [  0.25, 0.35, 0.55, 0.6, 0.65, 0.7, 0.75]
# lamda_2 = [0.1]
dropout = [0.1]
dp = 0.1
...
for l_1 in lamda_1:
	...
	for l_2 in lamda_2:
		for lr in [ 0.0005]:
			...
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu1,floatX=float32" python fix_enc_train_rcnn_gen.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) +' --train ../reviews.aspect1.train.txt.gz --dev ../reviews.aspect1.heldout.txt.gz --embedding ../word_vec.gz --load_rationale ../annotations.json --aspect ' + str(aspect) + \
				' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --debug '+ str(debug) +' --select_all ' +str(select_all) + ' --learning_rate '+str(lr)+' --save_model ' +_type+ "MODELS/"+model_file +' --load_model '+ load_model_file + ' --load_emb_only '+ str(load_emb_only)
