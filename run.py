...
lamda_1 = [0.000085, 0.000095] # for small 0.000085, 2 gru, markov, 
lamda_2 = [ 1, 2]
dropout = [0.1]
dp = 0.1
...
debug = 1
select_all = -1
output_file = 'rough2_new_gen_outputs_'+"linear_rcnn_0.0005"+'.json'
covered_percentage = []
graph_data_file = 'graph_data/data_vs_lamda_table_dummy.txt'
open(graph_data_file, 'w')


_type = 'JUST_OUTPUT_LAYER/'
...
for l_1 in lamda_1:
	for l_2 in lamda_2:
		for lr in [0.005, 0.0005]:
			...
			#load_model_file = 'models_new_gen/'+model_file #+str(trained_max_epochs-1)+'.pkl.gz'
			...
			#run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu0,floatX=float32" python gen_enc.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) + ' --embedding word_vec.gz --load_rationale annotations.json --aspect ' + str(aspect) + \
			#	' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --load_model ' + load_model_file +' --select_all '+ str(select_all)  
			
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu1,floatX=float32" python just_output_layer.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) + ' --embedding word_vec.gz --train reviews.aspect1.train.txt.gz --dev reviews.aspect1.heldout.txt.gz --load_rationale annotations.json --aspect ' + str(aspect) + \
				' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --debug '+ str(debug) +' --select_all ' +str(select_all) + ' --learning_rate '+str(lr)+' --save_model ' +_type+ "MODELS/"+model_file# +' --load_model '+ load_model_file
