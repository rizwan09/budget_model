...
class Encoder(object):
    ...
    def ready(self):
        ...
        # batch
        loss_mat = self.loss_mat  =  (preds-y)**2 #T.nnet.binary_crossentropy(preds, y)
        #for movie accuracy
...
def main():
    ...
    if args.load_model and not args.dev and not args.train:
        ...
        get_loss_and_pred = theano.function(
                ...
                outputs = [ model.encoder.loss_vec, model.encoder.preds, model.z ],
        ...
        # disable dropout
            ...
            if test_batches_y is not None:
                ...
                if test_batches_y is not None:
                            ...
                            test_obj, test_loss, test_diff, test_p1, test_accuracy = model.evaluate_data(
                                test_batches_x, test_batches_y, sample_encoder, sampling=True)
                            test_time = time.time() - start_rational_time
                            say(("\t accuracy={:0.3f} "+ 
                                    "  p[1]g={:.2f}, test time={} \n").format(
                            test_accuracy,
                            test_p1,
            ...
            #))
                ...
                data = str('%.3f' % test_accuracy) + "\t" + str('%.2f' %test_p1) + "\t" + str('%.5f' % test_time) +"\t" + str(args.sparsity) \
