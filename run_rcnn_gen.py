...
_type = '../RCNN_RCNN/Trained_MODELS/'
f = 0
for l_1 in lamda_1:
	# f+=1
	# if(f>4):exit()
	for l_2 in lamda_2:
		for lr in [ 0.0005]:
			...
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu1,floatX=float32" python rcnn_gen.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) +' --train ../reviews.aspect1.train.txt.gz --dev ../reviews.aspect1.heldout.txt.gz --embedding ../word_vec.gz --load_rationale ../annotations.json --aspect ' + str(aspect) + \
