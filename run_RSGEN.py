...
for l_1 in lamda_1:
	for l_2 in lamda_2:
		for lr in [0.01]:#, 0.0005]:
			...
			# SRGN = Reinforcment Adam generator
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu1,floatX=float32" python just_output_layer_RSGEN.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) + ' --embedding ../word_vec.gz --train ../reviews.aspect1.train.txt.gz --dev ../reviews.aspect1.heldout.txt.gz --load_rationale ../annotations.json --aspect ' + str(aspect) + \
