...
for l_1 in lamda_1:
	for l_2 in lamda_2:
		for lr in [0.01]:#, 0.0005]:
			f +=1
			# if(f==1):continue
			# if l_1 == 0.0003 and l_2 == 2: continue
			...
			#GGEN = Gold standard generator
			
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu1,floatX=float32" python just_output_layer_ggen.py --embedding ../word_vec.gz --load_rationale ../annotations.json --aspect ' + str(aspect) + \
				' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --debug '+ str(debug) +' --select_all ' +str(select_all) + ' --learning_rate '+str(lr)+' --save_model ' +_type+ "MODELS/"+model_file +' --load_model '+ load_model_file + ' --load_emb_only '+ str(load_emb_only)
