...
# lamda_2 = [0.01, 0.05, 0.1, -4, -5, -3, -2, -1, -.5, -0.1, 0, 0.25, 0.75, 0.85, 0.95, -0.25, -0.75, -0.85, -0.95, 0,1,-2,0.5, 0.75, -2.75, -3, 5.5, -6, -6.5]
...
lamda_2 = [1,-2,0.5, -2.75, -3, 5.5, -6, -6.5] #, 0.1,  0.2]
...
for l_1 in lamda_1:
	...
	for l_2 in lamda_2:
		for lr in [ 0.008]:
			...
			pyfile = 'rcnn_gen_lstm_enc_imdb.py'
			...
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu0,floatX=float32" python '+pyfile+' --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) +' --train imdb --dev imdb --test imdb  --embedding glove.6B.300d_w_header.txt' + \
				' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --debug '+ str(debug) +' --select_all ' +str(select_all) + ' --learning_rate '+str(lr)+' --save_model ' +_type+ "rcnn/MODELS/"+union+model_file \
				+ ' --load_model ' + _type +'MODELS/'+union+load_model_file +' --layer '+l+' --batch '+str(batch_size)+' -d '+str(d)#+ ' --load_gen_model ' + _type +'MODELS/'+union+load_model_file
			...
			run_command+= ' >> '+ _type+'rcnn/'+union+model_file + '.txt' 	
			print run_command
