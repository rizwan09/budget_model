...
<<<<<<< HEAD
            ...
            if is_glove:
                ...
                for word in embs.vocab:
                    if is_rt==True: word= word.encode("ascii")
                    assert word not in vocab_map, "Duplicate words in initial embeddings"
                    try: 
                        vocab_map[word.encode("ascii")] = len(vocab_map)
                    except:
                        vocab_map[word] = len(vocab_map)
                    # if is_rt==True:vocab_map[word.encode("ascii")] = len(vocab_map)
                    # else: vocab_map[word] = len(vocab_map)
                    emb_vals.append(embs.wv[word])
...
class LSTM(Layer):
    '''
        LSTM implementation.
    '''
    def __init__(self, n_in, n_out, activation=tanh, order = 1,
            clip_gradients=False):
