...
for l_1 in lamda_1:
	...
	for l_2 in lamda_2:
		for lr in [ 0.0009]:
			...
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu0,floatX=float32" python rcnn_gen_rt.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) +' --batch 50 -d2 128 --layer lstm --train rotten_tomatoes --dev rotten_tomatoes --test rotten_tomatoes  --embedding glove.6B.300d_w_header.txt' + \
				' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --debug '+ str(debug) +' --select_all ' +str(select_all) + ' --learning_rate '+str(lr)+' --save_model ' +_type+ "RCNN_RCNN/MODELS/"+model_file + ' --load_model ' + _type +'MODELS/'+union+load_model_file
