...
lamda_1 = [ 0.000085, 0.000095, 0.0001]
...
# lamda_1 = [ 0.00035]
lamda_2 = [  1, 0.8, 1.5, 0.5, 0.1]
# lamda_2 = [0.1]
dropout = [0.1]
dp = 0.1
trained_max_epochs = 10
load_emb_only = 1
max_epochs = 100
# learning = 'sgd'
aspect = 1
...
for l_1 in lamda_1:
	...
	for l_2 in lamda_2:
		for lr in [ 0.0005]:
			...
			run_command = ' THEANO_FLAGS="mode=FAST_RUN,device=gpu1,floatX=float32" python fix_enc_train_rcnn_gen.py --trained_max_epochs '+str(trained_max_epochs) +' --max_epochs '+ str(max_epochs) +' --embedding ../word_vec.gz --load_rationale ../annotations.json --aspect ' + str(aspect) + \
				' --dump ' + output_file +' --sparsity ' + str(l_1) +' --coherent ' + str(l_2) + ' --dropout '+ str(dp)+' --debug '+ str(debug) +' --select_all ' +str(select_all) + ' --learning_rate '+str(lr) +' --load_model '+ load_model_file #+ ' --load_emb_only '+ str(load_emb_only)
