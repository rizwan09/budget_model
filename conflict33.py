...
class Model(object):
    ...
    def train(self, train, dev, test, rationale_data, trained_max_epochs = None):
        ...
        for epoch_ in xrange(args.max_epochs ): # -50 when max_epochs  = 100 given
            ...
            if os.path.exists('../SVM/'+f):
                ...
                print 'loading generator finished'
            else:
            ...
            #             dev_batches_x, dev_batches_y, sample_generator_embs, clf, sample_encoder, sampling=True)
            ...
            if rationale_data is not None:
                ...
                r_mse, r_p1, r_prec1, r_prec2, gen_time, enc_time, prec_cal_time = self.evaluate_rationale(
                    rationale_data, valid_batches_x,
                    valid_batches_y, sample_generator_embs, clf, sample_encoder)
    ...
    def evaluate_data(self, batches_x, batches_y, sample_generator_embs, clf, eval_func, sampling=False):
        padding_id = self.embedding_layer.vocab_map["<padding>"]
        ...
        for bx, by in zip(batches_x, batches_y):
            if not sampling:
                ...
                bz = clf.predict(x2)
                bz=np.float64(bz).astype(theano.config.floatX) 
                a = len(bx)
                b = len(bx[0])
                bz = np.reshape(bz, (a,b))
                # print ' bz: ', bz, sum(bz)
    ...
    def evaluate_rationale(self, reviews, batches_x, batches_y, sample_generator_embs, clf, debug_func_enc):
